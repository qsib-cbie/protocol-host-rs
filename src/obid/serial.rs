...
pub trait ObidSerialSendable {
    ...
    fn deserialize(
        self: &Self,
        data: &[u8],
    ) -> Result<Box<dyn ObidSerialReceivable>, Box<dyn std::error::Error>>
    where
        Self: Sized;
...
pub mod advanced_protocol {
    use super::{calc_crc16, ObidSerialReceivable, ObidSerialSendable};
    ...
    impl ObidSerialSendable for HostToReader {
        fn serialize(self: &mut Self) -> std::vec::Vec<u8> {
            ...
            if self.data.len() > 0 {
                msg[5..(5 + self.data.len())].clone_from_slice(self.data.as_slice());
                // DATA
            }
            ...
            Self: Sized,
        {
            if data.len() < 7 {
                return Err(Box::new(std::io::Error::new(
                    ...
                    "Invalid (short) length for HostToReader message",
                )));
            }
            ...
            if calc_crc16(&data[..n_2]) != crc16 {
                return Err(Box::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    "Invalid CRC16 for HostToReader message",
                )));
            }
    ...
    impl ObidSerialReceivable for ReaderToHost {
        fn _serialize(self: &mut Self) -> std::vec::Vec<u8> {
            ...
            msg[5] = self.status;
            if self.data.len() > 0 {
                msg[6..(6 + self.data.len())].clone_from_slice(self.data.as_slice());
                // DATA
            }
...
mod tests {
    ...
    #[test]
    fn crc16_works() {
        let mut inventory_request = advanced_protocol::HostToReader::new(
            0,
            0xFF,
            0xB0,
        ...
        assert_eq!(
            0x4318,
            calc_crc16(&inventory_request[..inventory_request.len() - 2])
        );
    }
    ...
    fn host_to_reader_no_data() {
        ...
        let msg =
            advanced_protocol::HostToReader::_deserialize(marshalled_data.as_slice()).unwrap();
